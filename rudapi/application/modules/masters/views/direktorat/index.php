          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <select class="form-control" name="id_direktorat">
                ...
                <option value="<?php echo $d->id;?>"><?php echo ucfirst($d->nama);?></option>
                <?php }?>
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="situs" autocomplete="off" placeholder="Alamat URL">
            </div>

            <div class="form-group">
              <label>IP Lokal</label>
              <input type="text" class="form-control" name="ip_lokal" autocomplete="off" placeholder="IP Lokal">
            </div>
...
<script type="text/javascript">
    ...
    remoteSort: true,
    proxy: {
    url: '<?php echo base_url('master-direktorat'); ?>'
    }},
    trackOver: true,
    selModel: {
    type: 'rows',
    ...
    columns: [
        {
            ...
            <?php if ($this->datainclude->_deleted()) : ?>
              +('<a class="text-danger btn_hapus" href="javascript:;" onclick="Page.Delete(\'' + o.id + '\')" data-toggle="tooltip" data-html="true" title="Hapus Provinsi" style="margin-right:10px;"><i class="fa fa-minus-circle"></i><a>')
            <?php endif; ?>
    ...
    Page.Simpan = function(){
      ...
      if(nama == ''){
        ...
        $.ajax({
          url: '<?php echo base_url();?>masters/direktorat/tambah',
          type: 'POST',
    ...
    Page.Delete = function(id) {
      ...
      {
        $.ajax({
          url: '<?php echo base_url(); ?>masters/direktorat/hapus',
          type: 'POST',
          data: {id:id},
          success: function(response){
    ...
    Page.Edited = function(id_vd){
      console.log(id_vd);
      if(id_vd != '') {
        window.location.href = '<?php echo base_url(); ?>masters/direktorat/edited/' + id_vd;
      }
    };
    ...
    $(function(){
      ...
      $("input[name='keterangan']").on('input', function(e) {
        $(this).val($(this).val().replace(/[^a-z ]/g, ''));
      });
    });
