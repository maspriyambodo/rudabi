  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Form Tambah Kelurahan</h4>
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <select class="form-control" name="id_kecamatan">
                <option>Pilih</option>
                <?php foreach($this->datainclude->_kecamatan() as $p){?>
                <option value="<?php echo $p->id_kecamatan;?>"><?php echo $p->nama;?></option>
                <?php }?>
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="id_kelurahan" autocomplete="off" placeholder="ID Kelurahan">
            </div>

            <div class="form-group">
              <label>Nama Kelurahan</label>
              <input type="text" class="form-control" name="nama" autocomplete="off" placeholder="Nama Kelurahan">
            </div>

            <div class="form-group">
              <label>Latitude</label>
              <input type="text" class="form-control" name="latitude" autocomplete="off" placeholder="Latitude Kelurahan">
            </div>

            <div class="form-group">
              <label>Longitude</label>
              <input type="text" class="form-control" name="longitude" autocomplete="off" placeholder="Longitude Kelurahan">
            </div>
          </div>
          <!-- /.card-body -->
        </form>
...
<script type="text/javascript">
    ...
    columns: [
        {
            ...
            o.value = ''
              +('<a class="text-dark" href="javascript:;" onclick="Page.Detail(\'' + o.value + '\')" data-toggle="tooltip" data-html="true" title="Lihat Kelurahan" style="margin-right:20px;"><i class="fa fa-eye"></i><a>')
            <?php if ($this->datainclude->_edited()) : ?>
              +('<a class="text-warning btn_edit" href="javascript:;" onclick="Page.Edited(\'' + o.value + '\')" data-toggle="tooltip" data-html="true" title="Edit Kelurahan" style="margin-right:20px;"><i class="fa fa-edit"></i><a>')
            <?php endif; ?>
            <?php if ($this->datainclude->_deleted()) : ?>
              +('<a class="text-danger btn_hapus" href="javascript:;" onclick="Page.Delete(\'' + o.value + '\')" data-toggle="tooltip" data-html="true" title="Hapus Kelurahan" style="margin-right:10px;"><i class="fa fa-minus-circle"></i><a>')
            <?php endif; ?>
    ...
    Page.Simpan = function(){
      ...
      if(id_kecamatan == 'Pilih'){
        Swal.fire("Gagal!", "Nama Kecamatan Tidak Boleh Kosong.", "error");
      }else if(id_kelurahan == ''){
        Swal.fire("Gagal!", "ID Kelurahan Tidak Boleh Kosong.", "error");
      }else if(nama == ''){
        Swal.fire("Gagal!", "Nama Kelurahan Tidak Boleh Kosong.", "error");
      }else if(latitude == ''){
        ...
        $.ajax({
          url: '<?php echo base_url();?>masters/kelurahan/tambah',
          type: 'POST',
          data: {id_kecamatan:id_kecamatan, id_kelurahan:id_kelurahan, nama:nama, latitude:latitude, longitude:longitude},
          success: function(response){
            // console.log(response);
            $("#modal_tambah").modal('hide');
            Swal.fire("Berhasil!", "Data Berhasil Disimpan!","success").then( () => {
              location.href = '<?php echo base_url();?>masters/kelurahan';
            }
          )}
        });
      }
    ...
    // hapus data
    Page.Delete = function(id_kelurahan) {
      Swal.fire({
      ...
      {
        $.ajax({
          url: '<?php echo base_url(); ?>masters/kelurahan/hapus',
          type: 'POST',
    ...
    Page.Edited = function(id_kelurahan){
      // console.log(id_kelurahan);
      if(id_kelurahan != '') {
        window.location.href = '<?php echo base_url(); ?>masters/kelurahan/edited/' + id_kelurahan;
      }
    };

    // Inputan Tertentu
    $(function(){
      $("input[name='id_kelurahan']").on('input', function(e) {
