...
if (typeof jQuery != 'undefined') { 
	...
	jQuery(document).ready(function()
	{
		...
		jQuery("div.show-code-block").html('<div class="show-code-close-icon"><a href="javascript:void(0)"><img src="../assets/ui/images/close.gif" /></a></div><span id="titlebar">XML</span><div class="clear"></div><pre class="prettyprint"></pre><div class="show-code-close-btn"><a class="qua qua-button" href="javascript:void(0)"><span>Close</span></a></div>');


		jQuery("a.view-xml").unbind("click").click( function() {
			var chartDATA = '';
			jQuery("#titlebar").html("Chart XML Data");	
			chartDATA = chart.getChartData('xml').replace(/\</gi, "&lt;").replace(/\>/gi, "&gt;");
			showChartData(chartDATA);	
		});

		jQuery("a.view-json").unbind("click").click( function() {
										 
			var chartDATA = '';
			jQuery("#titlebar").html("Chart JSON Data");
		...
		jQuery('.show-code-close-icon a').unbind("click").click(function() {
			jQuery('.show-code-block').hide();
		});
		...
		if(NO_FLASH)
		{
			...
			jQuery(".qua-button-holder").css({"margin-left":"127px", "width":"350px"});
		}
...
function showChartData(data)
{
	...
	jQuery('.show-code-block').css('height', (jQuery(document).height() - 56) ).show();
	prettyPrint();
