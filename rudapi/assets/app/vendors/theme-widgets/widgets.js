...
var KanbanWidget = function(node) {
  ...
  var TaskModal = {
    view: function(vnode) {
      var ctrl = this.ctrl(vnode);
      return [
        m('div.modal.modal-primary', { style: { display: 'block'} }, [
          m('div.modal-dialog', [
            m('div.modal-content', [
              m('div.modal-header', [
                m('h4.modal-title.text-inverse', 'Task Title'),
                m('button.btn.btn-primary',{ onclick: ctrl.closeModal },[
                  m('span', [
  ...
  var Task = {
    view: function(vnode) {
      var ctrl = this.ctrl(vnode);
      return [
        m('div.task-item', { onclick: ctrl.loadTaskModal}, [
          ...
          ( vnode.attrs.task.members !== undefined && vnode.attrs.task.members.length ) ?  m('div.task-users', vnode.attrs.task.members.map( function(memberId) {
            ...
            return [
              m('figure.thumb-xs.d-inline-block', m('img.circle', { src: memberObj.avatar } ) )
            ];
          })) : null,
        ]),
      ];
